...
To see maintainers annotations for each check, use the `--show-ma` option. The annotations are configured as labels but parsed to a human-readable format in the output.
...
The available checks are the Scorecard checks in lower case e.g. Binary-Artifacts is `binary-artifacts`, and the available annotation reason options are as follows:
