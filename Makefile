...
.PHONY: all
all: build test checkprojects build-cron build-scripts verify  build-update
...
.PHONY: build-update
build-update: ## builds the scripts/update binary
	cd ./scripts/update && go mod tidy && go mod verify	
	cd ./scripts/update && CGO=0 go build .
	./scripts/tree-status
