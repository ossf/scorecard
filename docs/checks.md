...
- For Python tooling installed inside GitHub workflows or release scripts, prefer a small requirements file with exact versions and hashes, then install it with `python -m pip install --require-hashes -r <file>`. This avoids trusting a mutable package artifact during the build bootstrap step while keeping the workflow readable.
