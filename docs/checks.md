...
Requirements: Authorization token must have
[Admin access](https://github.community/t/enable-branch-protection-get-api-without-admin/14197)
to repository.
...
Note: Without an authorization with Admin access to the repository, the check
will return a failing score.
...
week during the previous 90 days, the project receives the highest score. Fewer
commits during that time period are scored on a sliding scale.
...
The checks works by looking for:
...
The check cannot detect if the "read-only" GitHub permission settings is
