...
func convertToRegex(pattern string) string {
	...
	for len(pattern) > 0 {
		...
		default:
			switch pattern[0] {
			case '*':
				// Equivalent to ".*"" in regexp,
				// but GitHub uses the File::FNM_PATHNAME
				// flag for the File.fnmatch syntax
				// the * wildcard does not match
				// directory separators (/).
				regexPattern.WriteString("[^/]*")
			case '?':
				// Matches any one character. Equivalent
				// to ".{1}" in regexp, see FNM_PATHNAME
				// note above.
