...
func (handler *graphqlHandler) init(ctx context.Context, owner, repo string) error {
	...
	if err := handler.client.Query(ctx, handler.data, vars); err != nil {
		return fmt.Errorf("error querying Github graphQL data: %w", err)
	}
...
func (handler *graphqlHandler) getMergedPRs() []clients.PullRequest {
	return handler.prs
}

func (handler *graphqlHandler) getDefaultBranch() clients.BranchRef {
	return handler.defaultBranchRef
}
