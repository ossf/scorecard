...
func setup(inputFile string) (zipHandler, error) {
	tempDir, err := os.MkdirTemp("", repoDir)
	if err != nil {
		return zipHandler{}, fmt.Errorf("test failed to create TempDir: %w", err)
	}
	tempFile, err := os.CreateTemp(tempDir, repoFilename)
	if err != nil {
		return zipHandler{}, fmt.Errorf("test failed to create TempFile: %w", err)
	}
	testFile, err := os.OpenFile(inputFile, os.O_RDONLY, 0o644)
	if err != nil {
		return zipHandler{}, fmt.Errorf("unable to open testfile: %w", err)
	}
	if _, err := io.Copy(tempFile, testFile); err != nil {
		return zipHandler{}, fmt.Errorf("unable to do io.Copy: %w", err)
	}
	zipHandler := zipHandler{
		tempDir:     tempDir,
		tempZipFile: tempFile.Name(),
		once:        new(sync.Once),
	}
	zipHandler.once.Do(func() {
		// We don't want to run the code in zipHandler.setup(), so if we execute zipHandler.once.Do() right
...
func TestExtractZip(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name:      "Basic",
			inputFile: "testdata/basic.zip",
			listfileTests: []listfileTest{
				{
					// Returns all files in the zip.
	...
	for _, testcase := range testcases {
		t.Run(testcase.name, func(t *testing.T) {
			...
			// Extract zip.
			if err := handler.extractZip(); err != nil {
