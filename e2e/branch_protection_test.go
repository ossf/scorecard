...
var _ = Describe("E2E TEST PAT:"+checks.CheckBranchProtection, func() {
	Context("E2E TEST:Validating branch protection", func() {
		It("Should get non-admin branch protection on other repositories", func() {
			...
			// New version.
			Expect(scut.ValidateTestReturn(GinkgoTB(), "branch protection accessible", &expected, &result, &dl)).Should(BeTrue())
		...
		It("Should fail to return branch protection on other repositories", func() {
			...
			// New version.
			Expect(scut.ValidateTestReturn(GinkgoTB(), "branch protection accessible none", &expected, &result, &dl)).Should(BeTrue())
			Expect(repoClient.Close()).Should(BeNil())
		})
		It("Should fail to return branch protection on other repositories patch", func() {
			...
			// New version.
			Expect(scut.ValidateTestReturn(GinkgoTB(), "branch protection accessible patch", &expected, &result, &dl)).Should(BeTrue())
			Expect(repoClient.Close()).Should(BeNil())
		})
	})
})
...
var _ = Describe("E2E TEST:"+checks.CheckBranchProtection+" (repo rules)", func() {
	Context("E2E TEST:Validating branch protection with repo rules", func() {
		It("Should be able to read repo rules", func() {
			...
			Expect(scut.ValidateTestReturn(GinkgoTB(), "repo rules accessible", &expected, &result, &dl)).Should(BeTrue())
			Expect(repoClient.Close()).Should(BeNil())
		})
	})
})
