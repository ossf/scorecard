...
var _ = Describe("E2E TEST:"+checks.CheckSecurityPolicy, func() {
	Context("E2E TEST:Validating security policy", func() {
		...
		It("Should return valid security policy at commitSHA", func() {
			...
			Expect(err).Should(BeNil())
			repoClient := githubrepo.CreateGithubRepoClient(context.Background(), logger, 30)
			err = repoClient.InitRepo(repo, "e0cb70344e46276b37d65824f95eca478080de4a", repoClient.GetCommitDepth())
			Expect(err).Should(BeNil())
		...
		It("Should return valid security policy for rust repositories at commitSHA", func() {
			...
			Expect(err).Should(BeNil())
			repoClient := githubrepo.CreateGithubRepoClient(context.Background(), logger, 30)
			err = repoClient.InitRepo(repo, "bab40cdd29d19e0638cf1301dfd355c52b94d1c0", repoClient.GetCommitDepth())
			Expect(err).Should(BeNil())
