...
func SAST(c *checker.CheckRequest) (checker.SASTData, error) {
	...
	pysaWorkflows, err := getPysaWorkflows(c)
	if err != nil {
		return data, err
	}
...
func getPysaWorkflows(c *checker.CheckRequest) ([]checker.SASTWorkflow, error) {
	var workflowPaths []string
	var sastWorkflows []checker.SASTWorkflow
	err := fileparser.OnMatchingFileContentDo(c.RepoClient, fileparser.PathMatcher{
		Pattern:       ".github/workflows/*",
		CaseSensitive: false,
	}, searchGitHubActionWorkflowPysa, &workflowPaths)
	if err != nil {
		return sastWorkflows, err
	}
	for _, path := range workflowPaths {
...
var searchGitHubActionWorkflowPysa fileparser.DoWhileTrueOnFileContent = func(path string,
	content []byte,
	args ...interface{},
) (bool, error) {
	if !fileparser.IsWorkflowFile(path) {
	...
	for _, job := range workflow.Jobs {
		for _, step := range job.Steps {
			...
			uses := strings.TrimPrefix(e.Uses.Value, "actions://")
			action, _, _ := strings.Cut(uses, "@")
			if strings.HasPrefix(action, "facebook/pysa-action") {
				*paths = append(*paths, path)
			}
		}
	}
