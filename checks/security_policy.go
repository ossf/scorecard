...
func SecurityPolicy(c *checker.CheckRequest) checker.CheckResult {
	...
	onFile := func(name string, dl checker.DetailLogger, data worker.FileCbData) (bool, error) {
	...
	err := worker.CheckIfFileExists(CheckSecurityPolicy, c, onFile, &r)
	if err != nil {
	...
	case err == nil:
		...
		err = worker.CheckIfFileExists(CheckSecurityPolicy, dotGitHub, onFile, &r)
		if err != nil {
			return checker.CreateRuntimeErrorResult(CheckSecurityPolicy, err)
		}
		if r {
