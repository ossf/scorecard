...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	if len(r.GitLabProtectedTags) == 0 && len(r.GitLabBranches) == 0 {
	...
	// Check if there are any release tags
	if len(r.Tags) == 0 {
		f, err := finding.NewWith(
			fs,
			Probe,
	...
	for i := range r.Tags {
		...
		// Find matching patterns for this tag
		matchingPatterns := findMatchingPatterns(tagName, r.GitLabProtectedTags)

		if len(matchingPatterns) == 0 {
			// No protection for this tag
			f, err := finding.NewWith(
				fs,
				Probe,
				fmt.Sprintf("release tag '%s' can be created by anyone with write access", tagName),
				nil,
			...
			f = f.WithValue(TagNameKey, tagName)
		...
		case minAccessLevel == AccessLevelNone:
			...
			text = fmt.Sprintf("release tag '%s' is fully protected: no one can create tags with this name", tagName)
		case minAccessLevel >= AccessLevelMaintainer:
			protectionLevel = "strong"
			outcome = finding.OutcomeTrue
			text = fmt.Sprintf("release tag '%s' is protected: only maintainers+ can create tags with this name", tagName)
		default:
			protectionLevel = "weak"
			outcome = finding.OutcomeFalse
			text = fmt.Sprintf("release tag '%s' has weak protection: developers can create tags with this name", tagName)
		}
