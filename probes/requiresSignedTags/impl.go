...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	for i := range r.Tags {
		...
		requiresSignatures := tag.TagProtectionRule.RequireSignatures != nil &&
			*tag.TagProtectionRule.RequireSignatures
		var text string
		var outcome finding.Outcome
		if requiresSignatures {
			text = fmt.Sprintf("tag '%s' requires signed tags", *tag.Name)
			outcome = finding.OutcomeTrue
		} else {
			text = fmt.Sprintf("tag '%s' does not require signed tags", *tag.Name)
