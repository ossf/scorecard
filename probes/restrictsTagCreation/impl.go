...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	for i := range r.Tags {
		...
		restrictsCreation := tag.TagProtectionRule.RestrictCreation != nil &&
			*tag.TagProtectionRule.RestrictCreation
		var text string
		var outcome finding.Outcome
		if restrictsCreation {
			text = fmt.Sprintf("tag '%s' restricts tag creation", *tag.Name)
			outcome = finding.OutcomeTrue
		} else {
			text = fmt.Sprintf("tag '%s' does not restrict tag creation", *tag.Name)
