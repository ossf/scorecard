...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	for i := range r.Tags {
		...
		allowsDeletion := tag.TagProtectionRule.AllowDeletions != nil &&
			*tag.TagProtectionRule.AllowDeletions
		var text string
		var outcome finding.Outcome
		if !allowsDeletion {
			text = fmt.Sprintf("tag '%s' blocks deletions", *tag.Name)
			outcome = finding.OutcomeTrue
		} else {
			text = fmt.Sprintf("tag '%s' allows deletions", *tag.Name)
