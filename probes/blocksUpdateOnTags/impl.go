...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	for i := range r.Tags {
		...
		allowsUpdate := tag.TagProtectionRule.AllowUpdates != nil &&
			*tag.TagProtectionRule.AllowUpdates
		var text string
		var outcome finding.Outcome
		if !allowsUpdate {
			text = fmt.Sprintf("tag '%s' blocks updates", *tag.Name)
			outcome = finding.OutcomeTrue
		} else {
			text = fmt.Sprintf("tag '%s' allows updates", *tag.Name)
