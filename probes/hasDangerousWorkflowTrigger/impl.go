...
//nolint:stylecheck
package hasDangerousWorkflowTrigger
...
const Probe = "hasDangerousWorkflowTrigger"
...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	for _, e := range r.Workflows {
		e := e
		if e.Type == checker.DangerousWorkflowSecretUsage {
			f, err := finding.NewWith(fs, Probe,
				fmt.Sprintf("workflow_run used to trigger a privileged workflow '%v'", e.File.Snippet),
