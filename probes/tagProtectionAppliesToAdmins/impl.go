...
package tagProtectionAppliesToAdmins
...
const (
	Probe      = "tagProtectionAppliesToAdmins"
	TagNameKey = "tagName"
)
...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	for i := range r.Tags {
		...
		enforcesAdmins := tag.TagProtectionRule.EnforceAdmins != nil &&
			*tag.TagProtectionRule.EnforceAdmins
		var text string
		var outcome finding.Outcome
		if enforcesAdmins {
			text = fmt.Sprintf("tag '%s' protection applies to admins", *tag.Name)
			outcome = finding.OutcomeTrue
		} else {
			text = fmt.Sprintf("tag '%s' protection does not apply to admins", *tag.Name)
